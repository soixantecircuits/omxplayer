...
// position. 
double udp_slave_sync(double pts)
{
    ...
    while (!master_exited || (!timed_out && master_exited < 0) && udp_master_position > 0) {
        double my_position = pts;
        printf("master_position: %f, local: %f, diff: %f\n", udp_master_position, my_position, udp_master_position - my_position);

        // if we're way off, seek to catch up
        dirty++;
        if (dirty == 10) 
          dirty = 0;
        if (abs(my_position - udp_master_position) > udp_seek_threshold && !dirty) 
        {
